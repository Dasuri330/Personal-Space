document.addEventListener('DOMContentLoaded', () => {
    const selectDrop = document.querySelector('#country'); // Use correct ID, it should be 'country'
    ...
    // Add event listener to show dropdown when clicked
    selectDrop.addEventListener('click', function() {
        this.setAttribute('size', '5'); // Set the size to a value that shows multiple options
    });

    // Remove the size attribute when the user clicks away
    document.addEventListener('click', function(event) {
        if (event.target !== selectDrop) {
            selectDrop.removeAttribute('size');
        }
    });
});
// 253 countries
